...
function Reporte() {
  ...
  try {
    ...
    // VERIFICACIÓN CRÍTICA: Si el blob es muy pequeño, leerlo como texto
    if (blob.size < 1000) {
      const blobText = await blob.text();
      console.log("Contenido del blob (primeros 500 chars):", blobText.substring(0, 500));
      
      // Si parece ser JSON (error del servidor)
          ...
          <div className="d-flex justify-content-center mb-4">
            <div className="position-relative" ref={selectRef}>
              ...
              <div className="custom-select-container-inline">
                ...
                >
                  {opcionesReporte.find((o) => o.value === tipoReporte)?.label}
                  <FaAngleDown className={`dropdown-icon ${selectAbierto ? "open" : ""}`} />
                </div>

                {selectAbierto && (
                  <div className={`custom-options-inline ${selectAbierto ? "open" : ""}`}>
                    {opcionesReporte.map((opcion) => (
                      <div
                        key={opcion.value}
                        onClick={() => {
                          setTipoReporte(opcion.value);
                          setFechaInicio(null);
                          setFechaFin(null);
                          setErrores({});
                          setMostrarVisor(false);
                        ...
                        className={tipoReporte === opcion.value ? "selected" : ""}
                      >
                        {opcion.label}
                      </div>
                    ))}
