  ...
  if (proyectosDesbloqueados.includes(idProyecto)) {
    ...
    try {
      ...
      if (data.success) {
        ...
        setProyectos(prev => prev.map(p => p.id_proyecto === idProyecto ? {...p, p_estatus: "En proceso"} : p));
      }
    } catch (error) {
...
const handleCompletarTarea = async (idTarea, idProyecto) => {
  try {
    const res = await fetch(`http://127.0.0.1:8000/api/tareas/${idTarea}/cambiar-status`, {
      method: 'PUT',
    });
    const data = await res.json();
    if (data.success) {
        ...
        prev.map(proy => {
          if (proy.id_proyecto === idProyecto) {
            const tareasActualizadas = proy.tareas.map(t =>
              t.id_tarea === idTarea ? { ...t, t_estatus: "Finalizada" } : t
            );
            const tareasCompletadas = tareasActualizadas.filter(t => t.t_estatus === "Finalizada").length;
            return { ...proy, tareas: tareasActualizadas, tareas_completadas: tareasCompletadas };
          }
          return proy;
        })
      );
  ...
  return (
    <div className="container-fluid p-0 dp-global">
      <Header />
      <div className="container my-4">

        <h1 className="form-titulo mb-3 text-center">Proyectos finalizados</h1>

        {/* Buscador */}
        <div className="buscador-verproyectos-contenedor mb-4">
...
<div className="dp-lista">
  {loading ? (
            ...
            <div className="dp-tareas-lista mt-3">
              {p.tareas.map(t => (
                <div key={t.id_tarea} className="dp-tarea-item">
                  ...
                  <input
                    type="checkbox"
                    checked={t.t_estatus === "Finalizada"}
                    disabled={t.t_estatus === "Finalizada"} // solo deshabilitado si ya está finalizada
    ...
    <div className="dp-empty-state text-center">
      <LuClock3 className="dp-empty-icon" />
      <h3>No hay proyectos en proceso</h3>
      <p>{busqueda ? 'No se encontraron proyectos con ese nombre' : 'Todos los proyectos están completados o no hay proyectos asignados'}</p>
    </div>
  )}
</div>
