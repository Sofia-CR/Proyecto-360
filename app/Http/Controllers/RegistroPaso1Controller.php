...
class RegistroPaso1Controller extends Controller
{
    public function validarInvitacion(Request $request)
    {
        ...
        if (!$invitacion || $invitacion->usado) {
            return response()->json(['ok' => false, 'message' => 'Invitación inválida o ya usada'], 404);
        }

        if ($invitacion->usuarios_registrados >= $invitacion->max_usuarios) {
            return response()->json(['ok' => false, 'message' => 'Se alcanzó el límite de registros'], 400);
        }
        ...
        try {
            ...
            return response()->json(['ok' => false, 'message' => 'Error al enviar correo: '.$e->getMessage()], 500);
        }
        ...
        return response()->json(['ok' => true, 'message' => 'Token enviado al correo']);
    }
}
